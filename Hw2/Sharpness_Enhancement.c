...
int (*Filter_c1)[3]; // 宣告一個指向3x3陣列的指標
int (*Filter_c2)[3]; // 宣告一個指向3x3陣列的指標
int Laplacian[3][3] = {{-1, -1, -1}, {-1, 9, -1}, {-1, -1, -1}};
int Unsharp_Masking[3][3] = {{0,-1, 0}, {-1, 5,-1}, {0,-1, 0}};
int Gaussian[3][3] ={{1, 1, 1}, {1, -7, 1}, {1, 1, 1}};
int Sobel_h[3][3] ={{-1, 0, 1}, {-2, 1, 2}, {-1, 0, 1}};
int Sobel_v[3][3] ={{-1, -2,-1}, {0, 1, 0}, {1, 2, 1}};
...
int main()
{
    ...
    scanf("%d", &f1);
    printf("Please enter Filter2 number:");
    scanf("%d", &f2);
    ...
    if (BitDepth != 32)
    {
        ...
        for (i = 1; i < Height - 1; i++)
        {
            for (j = 1; j < Width - 1; j++)
            {
                Y_new[i][j] = Filter_c1[0][0] * Y_origin[i - 1][j - 1] + Filter_c1[0][1] * Y_origin[i - 1][j] + Filter_c1[0][2] * Y_origin[i - 1][j + 1] +
                ...
                CB_new[i][j] = Filter_c1[0][0] * CB_origin[i - 1][j - 1] + Filter_c1[0][1] * CB_origin[i - 1][j] + Filter_c1[0][2] * CB_origin[i - 1][j + 1] +
                ...
                CR_new[i][j] = Filter_c1[0][0] * CR_origin[i - 1][j - 1] + Filter_c1[0][1] * CR_origin[i - 1][j] + Filter_c1[0][2] * CR_origin[i - 1][j + 1] +
        ...
        for (i = 0; i < Height; i++)
        {
            Y_new2[i][0] = Y_origin[i][0];
            Y_new2[i][Width - 1] = Y_origin[i][Width - 1];
            CB_new2[i][0] = CB_origin[i][0];
            CB_new2[i][Width - 1] = CB_origin[i][Width - 1];
            CR_new2[i][0] = CR_origin[i][0];
            CR_new2[i][Width - 1] = CR_origin[i][Width - 1];
        }
        for (j = 0; j < Width; j++)
        {
            Y_new2[0][j] = Y_origin[0][j];
            Y_new2[Height - 1][j] = Y_origin[Height - 1][j];
            CB_new2[0][j] = CB_origin[0][j];
            CB_new2[Height - 1][j] = CB_origin[Height - 1][j];
            CR_new2[0][j] = CR_origin[0][j];
            CR_new2[Height - 1][j] = CR_origin[Height - 1][j];
        }
        ...
        for (i = 1; i < Height - 1; i++)
        {
            for (j = 1; j < Width - 1; j++)
            {
                Y_new2[i][j] = Filter_c2[0][0] * Y_origin[i - 1][j - 1] + Filter_c2[0][1] * Y_origin[i - 1][j] + Filter_c2[0][2] * Y_origin[i - 1][j + 1] +
                               Filter_c2[1][0] * Y_origin[i][j - 1] + Filter_c2[1][1] * Y_origin[i][j] + Filter_c2[1][2] * Y_origin[i][j + 1] +
                               Filter_c2[2][0] * Y_origin[i + 1][j - 1] + Filter_c2[2][1] * Y_origin[i + 1][j] + Filter_c2[2][2] * Y_origin[i + 1][j + 1];
                CB_new2[i][j] = Filter_c2[0][0] * CB_origin[i - 1][j - 1] + Filter_c2[0][1] * CB_origin[i - 1][j] + Filter_c2[0][2] * CB_origin[i - 1][j + 1] +
                                Filter_c2[1][0] * CB_origin[i][j - 1] + Filter_c2[1][1] * CB_origin[i][j] + Filter_c2[1][2] * CB_origin[i][j + 1] +
                                Filter_c2[2][0] * CB_origin[i + 1][j - 1] + Filter_c2[2][1] * CB_origin[i + 1][j] + Filter_c2[2][2] * CB_origin[i + 1][j + 1];
                CR_new2[i][j] = Filter_c2[0][0] * CR_origin[i - 1][j - 1] + Filter_c2[0][1] * CR_origin[i - 1][j] + Filter_c2[0][2] * CR_origin[i - 1][j + 1] +
                                Filter_c2[1][0] * CR_origin[i][j - 1] + Filter_c2[1][1] * CR_origin[i][j] + Filter_c2[1][2] * CR_origin[i][j + 1] +
                                Filter_c2[2][0] * CR_origin[i + 1][j - 1] + Filter_c2[2][1] * CR_origin[i + 1][j] + Filter_c2[2][2] * CR_origin[i + 1][j + 1];
